...
-- Group establishments that appear more than once
SELECT company,
street,
city,
st,
...
-- Compare original vs backup tables
SELECT
...
-- Adding new column to copy information from another column into it
ALTER TABLE meat_poultry_egg_establishments ADD COLUMN st_copy text;
UPDATE meat_poultry_egg_establishments
SET st_copy = st;

-- No registers since columns are equal
SELECT st,
st_copy
FROM meat_poultry_egg_establishments
...
-- Concatenating '00' to zip codes with length 3
UPDATE meat_poultry_egg_establishments
SET zip = '00' || zip
WHERE st IN('PR','VI') AND length(zip) = 3;

-- Concatenating '0' to zip codes with length 4
UPDATE meat_poultry_egg_establishments
SET zip = '0' || zip
